...
public class Population {
	...
	private ValueEncodedChromosome[] population; // Array to store chromosome
													// objects

	private int[] rouletteWheel; // Used for selecting parents
	private int rouletteWheelSize; // Size of roulette wheel

	private float crossoverFraction; // Control how many offspring will be
										// created via crossover
	private float mutationFraction; // Control the amount of mutation per
									// evolution
	...
	// The closer a + 2*b + 3*c + 4*d is to 30 the lower the difference,
	// therefore fitness is high
	public int fitness(int a, int b, int c, int d) {
		// TODO: Calculate the difference between chromosome solution and
		// required equivalance of 30
		return (int) Math.abs((a + (2 * b) + (3 * c) + (4 * d)) - 30);
	}

	// If the fitness for the top chromosome is zero then we
	// have found a solution to a + 2b + 3c + 4d = 30
	public boolean converged() {
		// TODO: return boolean value based on whether we have found a solution
		// (True = converged, False = not converged)
		boolean result = false;

		for (int x = 0; x < population_size; x++) {
			if (population[x].getFitness() == 0) {
				result = true;
			}
		}
		return (result);
	...
	// Sort the population based on their fitness ASCENDING (lower number better
	// fitness)
	public void sort() {
		int i, j, first, temp;
	...
	public void evaluate() {
		// TODO: Loop through the population and set each chromosomes fitness
		// based on its values a,b,c,d
		for (int x = 0; x < population_size; x++) {
			population[x].setFitness(fitness(population[x].getGeneAt(0),population[x].getGeneAt(1),
					population[x].getGeneAt(2),population[x].getGeneAt(3)));
		}
	}
		...
		for (int i = num; i < population_size; i++) {
			...
			if (c1 != c2) {
				...
				for (int g = 0; g < chromosome_length; g++) {
					...
					// Copy from parent c1 after locus g
					else
	...
	public void mutate() {
		...
		for (int i = 0; i < num; i++) {
			int c = (int) (population_size * Math.random() * 0.99); // Random
																	// chromosome
			int g = (int) (chromosome_length * Math.random() * 0.99); // Random
																		// gene
	...
	public void removeDuplicates() {
		for (int i = population_size - 1; i > 3; i--) // i>3 dont touch the top
														// of the population
		{
			for (int j = 0; j < i; j++) {
	...
	public void display() {
		for (int j = 0; j < population_size; j++) {
			System.out.println(
					"Chromosome[" + j + "] = " + population[j].toString() + " Fitness = " + population[j].getFitness());
		}
	}
