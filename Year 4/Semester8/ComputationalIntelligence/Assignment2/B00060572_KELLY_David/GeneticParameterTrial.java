...
//1. Vary the *population_size* from 5 to 1000 while keeping the *crossover_fraction = 50%* and the *mutation_fraction = 20%*
		...
		int[] current_trial = POPULATION_TRIALS; // This is how you keep a fixed population size [2] = 15
		float mutation_param = MUTATION_TRIALS[2]; // This is how you keep a fixed mutation fraction [1] = 1%
		float crossover_param = CROSSOVER_TRIALS[5];
		String trial_name = "Population Size   ";
		...
		for(int i = 0; i < NUM_TRIALS; i++){
			...
			for(int j = 0; j < NUM_REPEATS_AVG; j++){
				...
				Population p = new Population(current_trial[i], CHROMOSOME_LENGTH, crossover_param, mutation_param);
...
//2. Vary the *crossover_fraction* from 0% to 100% while keeping the *population_size = 50* and the *mutation_fraction = 20%*
		...
		// Make a experimental TRIAL
		// Setup a particular trial - in this case we are fixing all parameters except the crossover fraction
		int population_param = POPULATION_TRIALS[4];
		float mutation_param = MUTATION_TRIALS[2];
		float[] current_trial = CROSSOVER_TRIALS;
		String trial_name = "Crossover Fraction   ";
		// NOTE: change this block for different trials/experiments
		// **************************************************************************************************
...
//3. Vary the *mutation_fraction* from 0% to 300% while keeping the *population_size = 50* and the *crossover_fraction = 50%*
		...
		// Setup a particular trial - in this case we are fixing all parameters except the crossover fraction
		int population_param = POPULATION_TRIALS[4];
		float[] current_trial = MUTATION_TRIALS;
		float crossover_param = CROSSOVER_TRIALS[5];
		...
		for(int i = 0; i < NUM_TRIALS; i++){
			...
			for(int j = 0; j < NUM_REPEATS_AVG; j++){
				...
				// Create a population using particular trial data
				Population p = new Population(population_param, CHROMOSOME_LENGTH, crossover_param, current_trial[i]);
